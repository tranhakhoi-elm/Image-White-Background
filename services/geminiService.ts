...
export const editProductImage = async (base64Image: string, prompt: string, modelName: string = 'gemini-3.1-flash-image-preview', imageSize: string = '2K'): Promise<string> => {
  const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
...
export const generateProductImage = async (settings: GenerationSettings, variantSeed: number): Promise<string> => {
  ...
  if (settings.visualStyle === "SCENE_STAGING") {
    ...
    if (settings.whiteBgWebPromptType === 'B') {
      finalPrompt = `
Ultra realistic lighting and shadow refinement for a single frying pan on white background.
...
STRICT PRESERVATION:
- Keep the exact original product (no change in shape, color, material, text, or proportions)
- Do NOT duplicate or add any objects
- Do NOT change composition or camera angle

Lighting:
- soft but directional key light from upper-left at 45 degrees
- clean commercial lighting (premium product photography style)
- gentle highlight along left edge and top lid
- smooth light transition across the body
- no harsh specular or blown highlights

Shadow:
- create a soft, natural shadow attached to the base of the bottle
- shadow direction: to the right and slightly backward
- short to medium length shadow (not too long)
- soft edge with subtle gradient fade
- darkest area close to the base, fading outward
- shadow shape follows cylindrical form of the bottle
